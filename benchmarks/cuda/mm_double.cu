...
__global__
void map2xy2D_kernel(double *arr1, double *arr2, int par, double *resp, int size, double (*f)(double*,double*,int,int,int))
{
...
__device__
double anonymous_9nl89mhko6(double *mat1, double *mat2, int m, int x, int y)
{
        double sum = 0.0;
for( int i = 0; i<m; i+=1){
...
void cpu_mm(double *h_a, double *h_b, double *h_result, int m, int n, int k) {
    for (int i = 0; i < m; ++i) 
    {
        for (int j = 0; j < k; ++j) 
        {
...
void checkElementsAre(double *gpu, double *cpu, int N)
{
  for(int i = 0; i < N; i++)
  {
    if(gpu[i] != cpu[i])
...
{   
    ...
    double *cpu_result = (double*) malloc(m*m*sizeof(double));
...
for (int i = 1; i <= m*m; ++i) {
  ...
  int block_size = 16;
   ...
   float time;
    ...
    cudaMalloc((void **) &d_c, sizeof(double)*m*m);
     j_error = cudaGetLastError();
    ...
    cudaMemcpy(d_b, b, sizeof(double)*m*m, cudaMemcpyHostToDevice);
     j_error = cudaGetLastError();
    if(j_error != cudaSuccess) printf("Error 5: %s\n", cudaGetErrorString(j_error));
    
    double (*f)(double*,double*,int,int,int) =  (double (*)(double*,double*,int,int,int)) get_anonymous_9nl89mhko6_ptr();
    ...
    cudaMemcpy(c, d_c, sizeof(double)*m*m, cudaMemcpyDeviceToHost);
    j_error = cudaGetLastError();
