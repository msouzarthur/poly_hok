  ...
  def map2(t1,t2,func) do
    ...
    type = PolyHok.get_type_gnx(t2)
     ...
     result_gpu = PolyHok.new_gnx(l,c, type)
      ...
      PolyHok.spawn_jit(&DP.map_2kernel/5,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[t1,t2,result_gpu,size,func])
...
vet2 = PolyHok.new_nx_from_function(1,n,{:f,32},fn -> 1.0 end )
...
result = ref1
    |> DP.map2(ref2, PolyHok.hok fn (a,b) -> a * b end)
    |> DP.reduce(0.0,PolyHok.hok fn (a,b) -> a + b end)
    |> PolyHok.get_gnx
